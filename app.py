...
ca = st.sidebar.slider("Number of Major Vessels", 0, 4)
...
if heart_btn:
    ...
    try:
        ...
        if pred == 1:
            st.error(f"⚠️ High Risk of Heart Disease ({prob*100:.2f}%)")
        else:
...
if stroke_btn:
    ...
    try:
        ...
        if pred == 1:
            st.error(f"⚠️ High Risk of Stroke ({prob*100:.2f}%)")
        else:
...
if both_btn:
    ...
    st.subheader("Overall Health Prediction")
